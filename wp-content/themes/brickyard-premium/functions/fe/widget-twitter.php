<?php
/**
 * Plugin Name: BrickYard Twitter Widget
...
?>
<?php 
add_action('widgets_init', create_function('', 'return register_widget("brickyardtwitter");'));
class brickyardtwitter extends WP_Widget {
	function brickyardtwitter() {
		$widget_ops = array('classname' => 'brickyardtwitter', 'description' => __('Twitter widget by BrickYard', 'brickyard') );
		$control_ops = array('width' => 200, 'height' => 400);
		$this->WP_Widget('brickyardttwi', __('BrickYard Twitter Widget', 'brickyard'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		if ( $instance ) {
			...
			$title = __( 'Join us on Twitter', 'brickyard' );
		} 
		...
		<p>
			<label for="<?php echo $this->get_field_id('username'); ?>">
				<?php _e('Twitter Username:', 'brickyard'); ?>
			</label>
			<input class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" type="text" value="<?php echo $username; ?>" />
		<p style="font-size: 10px; color: #999; margin: 0; padding: 0px;">
			<?php _e('Insert your Twitter username. (ex: brickyard).', 'brickyard'); ?>
		</p>
		</p>
		<?php 
	} 
	...
	function widget($args, $instance) {
		...
		foreach ($feed->get_items(0,$tweets) as $item) { ?>
			<p class="tweet-text"><?php echo brickyarduserurl(brickyardtagurl(brickyardmakeURL($item->get_description()))); ?> 
