<?php
/*
Plugin Name: BrickYard Tab Widget
...
class brickyard_widget extends WP_Widget {
	function __construct() {
        
		// ajax functions
		add_action('wp_ajax_brickyard_widget_content', array(&$this, 'ajax_brickyard_widget_content'));
		add_action('wp_ajax_nopriv_brickyard_widget_content', array(&$this, 'ajax_brickyard_widget_content'));
		...
		$widget_ops = array('classname' => 'widget_wpt', 'description' => __('Display popular posts, recent posts, comments and tags in tabbed format.', 'brickyard'));
		$control_ops = array('width' => 300, 'height' => 350);
		$this->WP_Widget('brickyard_widget', __('BrickYard Tab Widget', 'brickyard'), $widget_ops, $control_ops);
    }	

    function wpt_admin_scripts($hook) {
        wp_enqueue_script( 'brickyard_widget_admin', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wpt-admin.js', array( 'jquery' ), '1.0', true );
    }
    function wpt_register_scripts() { 
		// JS    
		wp_enqueue_script('brickyard_widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wp-tab-widget.js', array('jquery'), '1.0', true);     
		wp_localize_script( 'brickyard_widget', 'wpt',         
	...
	function form( $instance ) {
        ...
        <h4><?php _e('Select Tabs', 'brickyard'); ?></h4>
		...
		<div class="wpt_select_tabs">
			...
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_tags">
				...
				<?php _e( 'Tags Tab', 'brickyard'); ?>
			</label>
		</div>
        <div class="clear"></div>
        
        <h4 class="wpt_tab_order_header"><a href="#"><?php _e('Tab Order', 'brickyard'); ?></a></h4>
        ...
        <div class="wpt_tab_order" style="display: none;">
            ...
            <label class="alignleft" for="<?php echo $this->get_field_id('tab_order'); ?>_tags" style="width: 50%;">
			    ...
			    <?php _e('Tags', 'brickyard'); ?>
            </label>
        </div>
		<div class="clear"></div>
        
        <h4 class="wpt_advanced_options_header"><a href="#"><?php _e('Advanced Options', 'brickyard'); ?></a></h4>
        ...
        <p>
			<label for="<?php echo $this->get_field_id("allow_pagination"); ?>">				
				...
				<?php _e( 'Allow pagination', 'brickyard'); ?>
			</label>
		</p>
        ...
        <p>
			<label for="<?php echo $this->get_field_id('post_num'); ?>"><?php _e('Number of posts to show:', 'brickyard'); ?>
				<br />
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_thumb"); ?>">
				...
				<?php _e( 'Show post thumbnails', 'brickyard'); ?>
			</label>
		</p>   	
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_date"); ?>">	
				...
				<?php _e( 'Show post date', 'brickyard'); ?>	
			</label>	
		</p>
		...
		<p>		
			<label for="<?php echo $this->get_field_id("show_comment_num"); ?>">		
				...
				<?php _e( 'Show number of comments', 'brickyard'); ?>		
			</label>	
		</p>    
        ...
        <p>
			<label for="<?php echo $this->get_field_id('comment_num'); ?>">
				<?php _e('Number of comments on Comments Tab:', 'brickyard'); ?>
				<br />
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_avatar"); ?>">			
				...
				<?php _e( 'Show avatars on Comments Tab', 'brickyard'); ?>	
			</label>	
		</p>
	...
	function widget( $args, $instance ) {	
		extract($args);     
		extract($instance);    
		wp_enqueue_script('brickyard_widget');  
        ...
        $available_tabs = array('popular' => __('Popular', 'brickyard'), 
            'recent' => __('Recent', 'brickyard'), 
            'comments' => __('Comments', 'brickyard'), 
            'tags' => __('Tags', 'brickyard'));
            
        array_multisort($tab_order, $available_tabs);
        
		?>	
		<?php echo $before_widget; ?>	
		<div class="brickyard_widget_content" id="<?php echo $widget_id; ?>_content">		
	...
	function ajax_brickyard_widget_content() {     
		$tab = $_POST['tab'];       
		$args = $_POST['args'];  
		...
		switch ($tab) {        
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>	
								<div class="wpt-postmeta">						
									...
									<?php if ( $show_comment_num == 1 ) : ?>			
										<?php echo comments_number(__('No Comment','brickyard'), __('One Comment','brickyard'), '<span class="comments-number">%</span> '.__('Comments','brickyard'));?>				
									<?php endif; ?>						
								</div> <!--end .entry-meta--> 				
							<?php endif; ?>
						...
						<li>
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>			
								<div class="wpt-postmeta">										
									...
									<?php endif; ?>								
									<?php if ( $show_comment_num == 1 ) : ?>	
										<?php echo comments_number(__('No Comment','brickyard'), __('One Comment','brickyard'), '<span class="comm">%</span> '.__('Comments','brickyard'));?>									
									<?php endif; ?>		
								</div> <!--end .entry-meta--> 		
							<?php endif; ?>
			...
			case "comments":         
				...
				<ul>            
					...
					<?php endforeach; else : ?>           
						<li>                   
							<div class="no-comments"><?php _e('No comments yet.', 'brickyard'); ?></div>        
						</li>                             
    ...
    function tab_pagination($page, $last_page) {  
		...
		<div class="wpt-pagination">     
			<?php if ($page > 1) : ?>               
				<a href="#" class="previous"><span><?php _e('&larr; Previous', 'brickyard'); ?></span></a>      
			<?php endif; ?>        
			<?php if ($page != $last_page) : ?>     
				<a href="#" class="next"><span><?php _e('Next &rarr;', 'brickyard'); ?></span></a>      
			<?php endif; ?>          
		</div>                   
		<div class="clear"></div>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "brickyard_widget" );' ) );
